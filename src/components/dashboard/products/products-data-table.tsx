...
import {
  ...
  ProductChannelSyncStatusItem,
  ProductSyncStatusByProductId,
  ProductSyncStatusResponse,
} from "@/app/dashboard/products/sync-status";
...
type ProductsDataTableProps = {
  ...
  syncStatusByProductId: ProductSyncStatusByProductId;
};
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span
        className={cn(
          "text-sm",
          isZero ? "text-red-600" : isLow ? "text-amber-600" : "text-slate-700",
        )}
      >
...
const SYNC_STATUS_COPY: Record<ProductChannelSyncStatus, string> = {
  READY: "Elegivel",
  PENDING: "Na fila",
...
function syncStatusTone(status: ProductChannelSyncStatus) {
  if (status === "SYNCED") {
...
function ChannelSyncIcon({ status }: { status: ProductChannelSyncStatus }) {
  if (status === "SYNCED") {
...
function channelTitle(channel: ProductChannelSyncStatusItem) {
  return `${channelLabel(channel.source)}: ${
    SYNC_STATUS_COPY[channel.status]
  } - ${channel.message}`;
}

function ChannelSyncSummary({
  syncStatus,
}: {
  syncStatus?: ProductSyncStatusResponse;
}) {
  if (!syncStatus?.channels.length) {
    return <span className="text-xs text-slate-300">Sem canais</span>;
  }
      ...
      {syncStatus.channels.map((channel) => (
        <span
          ...
          className={cn(
            ...
            syncStatusTone(channel.status),
          )}
        >
          <ChannelSyncIcon status={channel.status} />
    ...
    <Card className="overflow-hidden border border-slate-100 shadow-sm">
      ...
      <div className="flex items-center justify-between gap-4 border-b border-slate-100 px-6 pt-1">
        ...
        <div className="flex gap-6">
          ...
          <TabLink href={tabHref("active")} active={activeTab === "active"}>
            Ativos
            <TabCount count={totalActive} />
          </TabLink>
          <TabLink href={tabHref("inactive")} active={activeTab === "inactive"}>
            Inativos
              ...
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <SortableHead
                    ...
                    currentSortDirection={sortDirection}
                  />
                  <TableHead className="px-4 py-3 text-xs font-medium text-slate-400">
                    Canais
                  </TableHead>
                  ...
                  >
                      ...
                      {hasPromotionalPrice(product) ? (
                        <div className="space-y-0.5">
                          <p className="text-sm font-semibold text-emerald-600">
                            {formatMoney(product.promotionalPriceCents!)}
                          </p>
                    ...
                    <TableCell className="px-4 py-3">
                      <StockCell
                        ...
                        reservedStockQuantity={product.reservedStockQuantity}
                      />
                    </TableCell>
                    ...
                    <TableCell className="px-4 py-3">
                      <ChannelSyncSummary
                        syncStatus={syncStatusByProductId[product.id]}
                      />
                    </TableCell>
